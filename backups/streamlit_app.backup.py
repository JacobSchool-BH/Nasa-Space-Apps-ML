...
def mission_ohe_matrix(missions: np.ndarray, mission_cats: list[str]) -> np.ndarray:
    """
    missions: shape (N,) array of strings
    mission_cats: list of categories in the exact training order
    Returns (N, len(mission_cats)) array with 0/1 one-hot.
    """
    missions = missions.astype(str)
    N = missions.shape[0]
    K = len(mission_cats)
    mat = np.zeros((N, K), dtype="float32")
    if K == 0: return mat
    # Build index map once
    idx = {cat: i for i, cat in enumerate(mission_cats)}
...
# ---------- Prediction (schema-stable) ----------
def predict_if_possible(df_input: pd.DataFrame):
    """
    Binary predictor (CONFIRMED vs FALSE POSITIVE) with schema-stable build:
    - Normalizes meta['feature_names'] even if stored oddly
    - Builds exact numeric/engineered block + manual mission one-hot
    - Creates any missing engineered columns before selection
    """
    ...
    if not isinstance(meta, dict):
        raise RuntimeError("Invalid meta.json (not a dict). Retrain once to re-create artifacts.")
    ...
    # feature_names could be a list, tuple, numpy array, OR (in some runs) a JSON string
    if isinstance(feat_names, np.ndarray):
        ...
        # try to parse JSON first; fallback to comma-split
        try:
            parsed = json.loads(feat_names)
            feat_names = parsed if isinstance(parsed, list) else [feat_names]
        except Exception:
            feat_names = [s.strip() for s in feat_names.split(",") if s.strip()]
    if not isinstance(feat_names, list) or not feat_names:
        raise RuntimeError("meta.json missing or malformed 'feature_names'. Retrain once so schema is saved.")
    ...
    if isinstance(mission_cats, np.ndarray):
        ...
        try:
            parsed = json.loads(mission_cats)
            mission_cats = parsed if isinstance(parsed, list) else [mission_cats]
        except Exception:
            mission_cats = [s.strip() for s in mission_cats.split(",") if s.strip()]
    mission_cats = [str(m) for m in mission_cats]

    # Split schema into numeric/engineered + mission one-hot (mission columns always end)
    mission_cols = [f"mission_{m}" for m in mission_cats]
    non_mission = [c for c in feat_names if not c.startswith("mission_")]
    # If feat_names already contains mission_*, keep order in feat_names; else append computed mission cols
    if any(c.startswith("mission_") for c in feat_names):
        num_feat_names = [c for c in feat_names if not c.startswith("mission_")]
        mission_cols = [c for c in feat_names if c.startswith("mission_")]
        feat_names = num_feat_names + mission_cols
    else:
        num_feat_names = non_mission
        feat_names = num_feat_names + mission_cols

    # ---- Normalize incoming data & build engineered features ----
    if isinstance(df_input, pd.Series):
        ...
        try:
            ...
            raise RuntimeError("Input must be a DataFrame/Series or dict-like structure.")

    df_fe, _ = build_features(df_input)

    # Ensure 'mission' exists and is string
    if "mission" not in df_fe.columns:
        df_fe["mission"] = "unknown"
    df_fe["mission"] = df_fe["mission"].astype(str)

    # --- Create any missing engineered numeric columns BEFORE selecting them ---
    for col in num_feat_names:
        ...
        df_fe[col] = df_fe[col].fillna(med)

    # Build numeric matrix with a CLEAN Python list key
    select_cols = [str(c) for c in num_feat_names]
    try:
        X_num = df_fe.loc[:, select_cols].astype("float32").to_numpy()
    except Exception as e:
        # Helpful diagnostics
        have = set(map(str, df_fe.columns))
        missing = [c for c in select_cols if c not in have]
        raise RuntimeError(
            f"Failed to build numeric feature matrix.\n"
            f"Requested {len(select_cols)} cols; first 5: {select_cols[:5]}\n"
            f"Missing {len(missing)} cols; first 5: {missing[:5]}\n"
            f"Original error: {e}"
        )

    # Manual one-hot for mission
    missions = df_fe["mission"].to_numpy()
    X_cat = mission_ohe_matrix(missions, mission_cats).astype("float32") if mission_cats else np.zeros((len(df_fe), 0), dtype="float32")

    # Concatenate in the exact order: numeric/engineered then mission one-hot
    X = np.hstack([X_num, X_cat]) if X_cat.shape[1] > 0 else X_num

    # Align with expected length
    expected = len(feat_names)
    got = X.shape[1]
    if got != expected:
        if got < expected:
            pad = np.zeros((X.shape[0], expected - got), dtype=X.dtype)
            X = np.hstack([X, pad])
        else:
            X = X[:, :expected]
    ...
    try:
        ...
        raise RuntimeError(f"Scaler transform failed (X shape={X.shape}). Error: {e}")
    ...
    label_names = label_names if isinstance(label_names, list) and len(label_names)==2 else ["FALSE POSITIVE","CONFIRMED"]
...
def main():
    ...
    if mode.startswith("Novice"):
        ...
        # Predictions (visible errors)
        pred_error = None
        preds = confs = None
        try:
            preds, confs = predict_if_possible(rows)
        except Exception as e:
            pred_error = f"Prediction error: {e}"

        if preds is None or confs is None:
            rows["predicted"] = np.nan; rows["pred_conf"] = np.nan
            if not ok_model:
                st.warning("Model unavailable: train a binary model in the **Researcher** tab.")
            elif pred_error:
                st.error(pred_error)
        else:
            rows["predicted"] = preds; rows["pred_conf"] = confs
        ...
        def _fmt_hover(row, idx):
            name = row["name"] if "name" in row else f"OBJ-{idx}"
            disp = str(row.get("disposition",""))
            pred = row.get("predicted", None)
            conf = row.get("pred_conf", None)
            if pd.notna(pred) and pd.notna(conf):
                pred_line = f"Model: {pred} ({float(conf)*100:.1f}%)"
            elif pd.notna(pred):
                pred_line = f"Model: {pred}"
            else:
                pred_line = "Model: (unavailable)"
            if disp.upper() == "CANDIDATE":
                top = f"{name} • Candidate"
            else:
                top = f"{name} • Catalog: {disp}"
            return f"{top}<br>{pred_line}"
